            ...
            html.Div(
                [
                    ...
                    dcc.Checklist(id='rating-95',
                                  options=[{'label': 'Only rating >= 95 ', 'value': 'Y'}]),
                    html.Div([
                        html.H5('Price Slider'),
                        dcc.RangeSlider(
                            id='price-slider', min=min_p, max=max_p,
...
@app.callback(Output('province-drop', 'options'), [Input('country-drop', 'value')])
...
@app.callback(Output('variety-drop', 'options'), [Input('province-drop', 'value')])
